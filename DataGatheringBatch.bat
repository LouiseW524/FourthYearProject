echo on
title DataGathering
:: Accessing API and Downloading dataset
cd C:/Users/louis/AppData/Local/Programs/Python/Python36-32/Scripts && kaggle config path -p C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1" && kaggle datasets download -d shubhmamp/english-premier-league-match-data
:: Unzipping the dataset
cd C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/datasets/shubhmamp/english-premier-league-match-data && dir &&  C:/"Program Files"/7-Zip/7z x datafile.zip
:: Renaming files in dataset, and moving them to dataset, for use in Python Scripts
cd season14-15
ren *.?* ????????????????????????????????????????????????????????"14-15".*
move *.json C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/datasets
cd..
cd season15-16
ren *.?* ????????????????????????????????????????????????????????"15-16".*
move *.json C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/datasets
cd..
cd season16-17
ren *.?* ????????????????????????????????????????????????????????"16-17".*
move *.json C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/datasets
cd..
cd season17-18
ren *.?* ????????????????????????????????????????????????????????"17-18".*
move *.json C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/datasets
cd C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/datasets
::Deleting old, obsolete folder, now empty
rd /s /q "shubhmamp" 
::Running python script
C:/Users/louis/AppData/Local/Programs/Python/Python36-32/python C:/"Program Files (x86)"/Jenkins/workspace/"Fourth Year Project V1.1"/JSONCleanerLine.py