...
cur.execute("""SELECT * FROM training_player_match_stats where playerposition LIKE '0' OR playerposition LIKE '1'""")
rows = cur.fetchall()
for row in rows:
    best_players = []
    total_points = 0
    cur.execute("""SELECT * FROM training_player_match_stats where matchid = %s ORDER BY playerrating DESC LIMIT(3)""", (row[1],))
    ...
    cur.execute("""INSERT INTO training_player_points_per_match ( playerid , matchid , playerpoints) VALUES (%s, %s, %s)""",(row[0], row[1], total_points))


cur.execute("""SELECT * FROM training_player_match_stats where playerposition LIKE '2'""")
...
for mids in midfielders:
    ...
    cur.execute("""SELECT * FROM training_player_match_stats where matchid = %s ORDER BY playerrating DESC LIMIT(3)""",(mids[1],))
    top_players = cur.fetchall()
    for player in top_players:
        if player[0] == mids[0]:
            best_players.append(player)
    ...
    cur.execute("""INSERT INTO training_player_points_per_match ( playerid , matchid , playerpoints) VALUES (%s, %s, %s)""",(mids[0], mids[1], total_points))

cur.execute("""SELECT * FROM training_player_match_stats where playerposition LIKE '3'""")
...
for fw in forwards:
    ...
    cur.execute("""SELECT * FROM training_player_match_stats where matchid = %s ORDER BY playerrating DESC LIMIT(3)""",(fw[1],))
    top_players = cur.fetchall()
    for player in top_players:
        if player[0] == fw[0]:
            best_players.append(player)
    ...
    cur.execute("""INSERT INTO training_player_points_per_match ( playerid , matchid , playerpoints) VALUES (%s, %s, %s)""",(fw[0], fw[1], total_points))

cur.execute("""SELECT * FROM training_player_match_stats where playerposition LIKE '4'""")
subs = cur.fetchall()
for sub in subs:
    cur.execute("""INSERT INTO training_player_points_per_match ( playerid , matchid , playerpoints) VALUES (%s, %s, %s)""",
                (sub[0], sub[1], 0))